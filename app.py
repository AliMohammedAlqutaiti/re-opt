...
st.subheader("🤖 تقرير تحليل الوكيل الذكي (Gemini Pro AI Agent)")

if not gemini_api_key:
    st.warning("⚠️ يرجى إدخال مفتاح Gemini API Key في الشريط الجانبي لتفعيل تقرير الوكيل الذكي.")
else:
    if st.button("توليد التقرير التحليلي العميق باستخدام Gemini Pro"):
        with st.spinner("الوكيل الذكي (Pro) يقوم بالتحليل المعمق وإعداد التقرير التشغيلي..."):
            try:
                client = genai.Client(api_key=gemini_api_key)
                
                prompt = f"""
                أنت وكيل ذكاء اصطناعي خبير في تشخيص و هندسة محطات الطاقة الشمسية.
                ...
                قدم تقريراً تشغيلياً واحترافياً متقدماً باللغة العربية يتضمن:
                1. تحليل السبب الجذري والتأثير الهندسي.
                2. التقييم المالي والأثر الاقتصادي الدقيق.
                3. توصية تنفيذية استراتيجية لصناع القرار متى يتم جدولة التنظيف والصيانة.
                """
                ...
                st.success("تم توليد التقرير بنجاح بواسطة Gemini Pro!")
