...
albedo = st.sidebar.slider("معامل الانعكاس والأرضية (Albedo)", min_value=0.1, max_value=1.0, value=0.2, step=0.05)
tilt_error = st.sidebar.slider("خطأ زاوية الميل (Degrees °)", min_value=0.0, max_value=90.0, value=5.0, step=1.0)
temp_loss = st.sidebar.slider("فقدان الحرارة الزائدة (%)", min_value=0.0, max_value=100.0, value=4.0, step=0.5)
...
def run_simulation(capacity, soiling_pct, alb, tilt_err, t_loss):
    ...
    # دمج نسب الفقد مع ضبط خطأ الميل ليعبر عن نسب مئوية مؤثرة
    total_degradation_pct = soiling_pct + (tilt_err * 0.5) + t_loss
...
col3.metric("مستوى كفاءة الألواح الفعلي", f"{max(0.0, 100.0 - (soiling_loss + tilt_error*0.5 + temp_loss)):.1f}%")
    ...
    if st.button("توليد التقرير التحليلي المتكامل"):
        with st.spinner("الوكيل الذكي يعالج بيانات الأداء والتشخيص عبر Interactions API..."):
            try:
                ...
                - فقدان الحرارة الزائدة: {temp_loss}%
                ...
                1. تحليل الأسباب الجذرية وتأثير نسب الفقد العالية المدخلة.
                2. التقييم المالي والأثر الاقتصادي لهذه المشاكل مجتمعة بالاعتماد على التعرفة المدخلة.
                3. توصيات هندسية تنفيذية واضحة لصناع القرار لمعالجة الأزمات التشغيلية.
                """
